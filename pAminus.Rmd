...
```{r}
(stats %<>%
  mutate(file = sub('^norm_xPappAminus_', '', file) %>% 
           sub('_BGsub', '', .) %>%
...
```{r}
stats %>%
  mutate(sum_pAminus_inferred = sum_pAplusandminus_signal - sum_pAplus_signal,
         pAminus_difference = sum_pAminus_signal - sum_pAminus_inferred) %>%
