...
(effective_body_and_genomic_pA_counts <- read.table('/Users/schmidm/Documents/Results/Lexogen_RNAseq/anno/genomic_pA_per_cleaned_body.txt', col.names=c('chr', 'start', 'end', 'id', 'pAfilter_count'), stringsAsFactors = FALSE) %>%
   mutate(pAfilter_count = ifelse(pAfilter_count == '.', 0, as.integer(pAfilter_count)),
          body_len = end-start,
          pAfiltered_cleaned_body_len = body_len - pAfilter_count) %>%
...
```{r}
body_len_NA_ids <- filter(anno, is.na(pAfiltered_cleaned_body_len)) %$% id %>% unique
```

```{r}
length(body_len_NA_ids)
```
actually quite a few transciption units have this

```{r}
table(anno$type[anno$id %in% body_len_NA_ids])
```
mostly CUTs, many SUTs, some snRNAs very few ORFs.  


```{r}
filter(both_df, part == 'body', id %in% body_len_NA_ids)
```

```{r}
filter(both_df, part == 'end', id %in% body_len_NA_ids)
```
