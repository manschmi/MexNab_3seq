...
```{r}
withXUTs_both_df <- bind_rows(both_df, XUTs_both_df) %>%
  mutate(type=factor(type, levels=c('ORF-T', 'XUT', 'SUTs', 'CUTs', 'snRNA')))
```
...
```{r violins Mex and Nab allpA rapa0 rep1 with XUTs cleaned}
withXUTs_both_df %>%
  filter(rep == 1, rapa == 0, type != 'other') %>%
  violin_plot(.)
```

```{r violins Mex and Nab pAplus and minus rapa0 rep1 with XUTs cleaned}
withXUTs_both_df %>%
  filter(rep == 1, rapa == 0, type != 'other', pA != 'pA+ + pA-', pA != 'pA-X') %>%
  violin_plot(.)
```
